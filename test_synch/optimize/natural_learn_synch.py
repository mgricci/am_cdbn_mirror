...
def weight_normalize(net, across_features=False):
    ...
    if across_features:
        net.conv1.weight.data.div_(l2_norm(net.conv1.weight.data))
    else:
	for f in net.num_features:
            net.conv1.weight.data[f,:,:,:].div_(l2_norm(net.conv1.weight.data[f,:,:,:]))
...
def run(net,dl, num_epochs=128,num_inner_steps=32,batch_size=64, lrs=[1e-2,1e-2, 1e-2], show_every=10, train=True):
    ...
    for t in tqdm(range(num_epochs)):
	    ...
	    cplx_batch = init(batch).cuda()
	    net.conv1.weight.requires_grad = False 
            optim_batch = optimize_v(net, cplx_batch, lr2,constraint=batch,num_steps=num_inner_steps, return_progress=False)
	    #plt.plot(a)
	    #plt.savefig('/home/jk/matt/nat_learn_synch/prog_{}.png'.format(b))
	    #plt.close()
	    if train:
	        net.conv1.weight.requires_grad = True
	        optim_activ = net.forward(optim_batch)
	        optim_activ.backward()
		 ...
		 optim_activ = net.forward(optim_batch)
	    ma.append(optim_activ.cpu().data.numpy())
	    if t%show_every == 0:
	        disp(net,ma,ol,t, batch=optim_batch.data.cpu().numpy())
    print('hey')
...
if __name__ == '__main__':
    ...
    load = False
    train = True
    net = deep if net_type is 'deep' else shallow    
    lrs = [1e-4,1e-1, 0.0]
    num_features=16
    num_epochs = 512
    num_inner_steps = 16
