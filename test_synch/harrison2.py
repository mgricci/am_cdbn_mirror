...
def Ham(z):
    ...
    return np.sqrt(np.sum(np.abs(np.conj(drive)*hidden)**2))

def constraint(z, i):
    units = z[:2*v_side**2] if i < v_side**2 else z[2*v_side**2:]
    side = v_side if i < v_side**2 else h_side
    ind = np.unravel_index(i, (side, side))
    c = clamp[ind] if i < v_side**2 else 1
    real = units[:side**2].reshape(side, side)
    imag = units[side**2:].reshape(side, side)
    cplx = real + 1j*imag
    y = cplx[ind]
    return c - np.sqrt(np.abs(y))

def init(v_side,h_side,i):
    v0 = 1.0*clamp*np.exp(1j*2*np.pi*np.random.rand(v_side, v_side))
    v0_real = np.real(v0)
    v0_imag = np.imag(v0)

    h0 = 1.0*np.exp(1j*2*np.pi*np.random.rand(h_side, h_side))
    h0_real = np.real(h0)
    h0_imag = np.imag(h0)
